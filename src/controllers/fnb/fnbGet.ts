...
export const fnbGet = async (req: express.Request, res: express.Response) => {
  try {
    ...
    if (Number.isNaN(limit) === true || Number.isNaN(page) === true || searchValue === undefined || availability === undefined) {
      ...
      const results = await fnbPaginatedQuery(page, limit, availability as string, searchValue as string, priceFilter as string, dateFilter as string, categoryFilter as string);
